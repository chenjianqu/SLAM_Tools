...
bool BoxEncloseStereoPointFactor::Evaluate(const double *const *parameters, double *residuals,
    ...
    if (jacobians){
        ...
        /*N_p.row(0) << e.x()/std::abs(e.x()) ,e.x()/std::abs(e.x()),e.x()/std::abs(e.x());
        N_p.row(1) << e.y()/std::abs(e.y()) ,e.y()/std::abs(e.y()),e.y()/std::abs(e.y());
        N_p.row(2) << e.z()/std::abs(e.z()) ,e.z()/std::abs(e.z()),e.z()/std::abs(e.z());*/
        ...
        if (jacobians[0]){
             ...
             */
            Mat36d jaco_oj;
            //jaco_oj.leftCols<3>() = -N_p.cwiseProduct(R_ojw) ; //对position的雅可比
            jaco_oj.leftCols<3>() = -N_p * R_ojw ; //对position的雅可比
            //jaco_oj.rightCols<3>() = - N_R * hat( R_ojw*(pts_w - P_woj) ).matrix();
            jaco_oj.rightCols<3>().setZero();//对orientation的雅可比
...
bool BoxPoseNormFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    cout<<"P_woi:"<<P_woi.transpose()<<endl;
    cout<<"err_t:"<<err_t.transpose()<<endl;
    ...
    if(jacobians){
        ...
        if(jacobians[0]){
            ...
            jacobian_pose_woi.leftCols(3) =  2* err_t.transpose();

            cout<<"jacobian_pose_woi:\n"<<jacobian_pose_woi<<endl;
