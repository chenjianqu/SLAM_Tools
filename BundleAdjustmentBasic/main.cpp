...
void bundleAdjustmentGaussNewton(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int i = 0; i < points_3d.size(); i++) {
            ...
            Eigen::Vector2d proj(fx * pc[0] / pc[2] + cx, fy * pc[1] / pc[2] + cy); //投影点

            Eigen::Vector2d e = points_2d[i] - proj;//投影误差

            cost += e.squaredNorm();//投影误差的二范数
            //雅可比矩阵,书上的公式7.46
...
public:
    ...
    virtual void linearizeOplus() override {
        ...
        const VertexPose *v = static_cast<VertexPose *> (_vertices[0]);
        Sophus::SE3d T = v->estimate();
        Eigen::Vector3d pos_cam = T * _pos3d;//相机坐标系的3D坐标
        double fx = _K(0, 0);
        double fy = _K(1, 1);
        double cx = _K(0, 2);
        double cy = _K(1, 2);
