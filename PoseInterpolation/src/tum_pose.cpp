...
vector<TumPose> TumPose::ReadOdomTxt(const fs::path &txt_path)
{
...
void TumPose::WriteOdomTxt(const fs::path &txt_path,const vector<TumPose> &poses){
...
vector<double> TumPose::ReadTimeStampTxt(const fs::path &txt_path) {
    vector<double> outputs;

    std::ifstream infile(txt_path, std::ios::in);
    if (!infile.is_open()){
        cout << "ReadTimeStampTxt() 读取文件失败" << endl;
        return {};
    }

    string line;
    while (getline(infile,line)){
        outputs.push_back(std::stod(line));
    }
