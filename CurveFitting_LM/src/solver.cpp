...
namespace mysolver
{
    Solver::Solver(ProblemType problemType) :
    ...
    void Solver::LogoutVectorSize() {
        std::cout <<
                  "1 problem::LogoutVectorSize verticies_:" << verticies_.size() <<
                  " edges:" << edges_.size()<<std::endl;
    }

    bool Solver::AddVertex(std::shared_ptr<Vertex> vertex) {
        if (verticies_.find(vertex->Id()) != verticies_.end()) {//已经存在该顶点了
            return false;
        } else {
            verticies_.insert(
                    std::pair<unsigned long, std::shared_ptr<Vertex>>(vertex->Id(), vertex)
                    );
        }
        return true;
    }

    bool Solver::AddEdge(std::shared_ptr<Edge> edge){
        if (edges_.find(edge->Id()) != edges_.end()) {//已经存在该边
            return false;
        } else {
    ...
    bool Solver::Solve(int iterations) {
        ...
        while (!stop && (iter < iterations))
        {
            ...
            while (!oneStepSuccess)  // 不断尝试 Lambda, 直到成功迭代一步
            {
                ...
                // 第四步，解线性方程 H*delta_X = B
                SolveLinearSystem();
                //Hession矩阵去掉阻尼因子,因为阻尼因子每次都不一样
...
//        delta_x_ = H.ldlt().solve(b_);
    ...
    void Solver::AddLambdatoHessianLM()
    {
        ...
        ulong size = Hessian_.cols();
        for (ulong i = 0; i < size; ++i) {
            Hessian_(i, i) += currentLambda_; //在对角线上加上阻尼因子
        }
    }
    ...
    bool Solver::IsGoodStepInLM()
    {
        ...
        if (rho > 0 && std::isfinite(tempChi))   //误差在下降
        {
