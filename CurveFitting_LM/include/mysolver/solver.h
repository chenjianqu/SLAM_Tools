...
#ifndef CURVEFITTING_LM_SOLVER_H
#define CURVEFITTING_LM_SOLVER_H
...
namespace mysolver
{
    ...
    public:
        ...
        Solver(ProblemType problemType);
    ...
    private:
        ...
        /// 所有的顶点
        std::map<unsigned long, std::shared_ptr<Vertex>> verticies_;

        /// 所有边
        std::unordered_map<unsigned long, std::shared_ptr<Edge>> edges_;

        /// 由vertex id查询edge
        std::unordered_multimap<unsigned long, std::shared_ptr<Edge>> vertexToEdge_;
        ...
        std::map<unsigned long, std::shared_ptr<Vertex>> verticies_marg_;
...
#endif //CURVEFITTING_LM_SOLVER_H
